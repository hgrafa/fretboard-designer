...
- `SpeedControl` / `VolumeControl`: dark popovers wrapping a `PlayerSlider`. Props `value`, `onChange`, `onOpenChange` (lets the player suppress auto-minimize while a panel is open). Speed uses `scale="log"` so `1×` sits centred between `.5` and `2`.
